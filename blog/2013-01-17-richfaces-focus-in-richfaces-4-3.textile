...
The @<rich:focus />@ component allows you to focus on a form input once the user requests the JSF page update (either full-page load or AJAX partial update). The @<rich:focus />@ will make sure that the focused component is the best choice for the user based on following criteria:

* validation results
* @tabindex@ settings
* position in the page
* which component was selected before the request
...
A default behavior is that the chosen component is focused each time an initial request or a postback is finished.

h4. Validation Awareness

The @<rich:focus />@ component chooses the right input based on validation results. The input component which has failed to be validated will be focused first.
...
The focusing priority given by the placement in the DOM can be influenced by giving a selected component @tabindex@ attribute with a value higher than 0.
...
In such case, user shouldn't be forced to focus input box himself. You can focus the input for him:
...
Sometimes it is too early to focus on component after the JSF request.
...
When all of the methods illustrated above do not fit your needs, then you have last option: you can leverage @FocusManager@ API.
