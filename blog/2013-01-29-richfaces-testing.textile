...
We came to a conclusion that we are doing *testing too late after a commit*. Certainly, we do lot of unit-testing and we write many integration tests - however they don't guard us from real bugs. They cover just a small part of a logic which is finally executed between a client and a server.

h3. The Integration Environment Complexity

The problem which we face is given by a complexity of the client-server nature of web frameworks like the JSF. A testing of the interaction is inherently so complex that it is often avoided. But while we are avoiding its testing, we can easily underestimate its importance.
...
Tests we usually write can be easily unit test and so we test regularly both the client-side and the server-side code.
